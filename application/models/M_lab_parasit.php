<?php 
class M_lab_parasit extends CI_Model{
...
$this->datatables->where('disposisi.nama_distribusi','Lab Parasit');
...
$this->db->insert('lab_parasit',$data);    
}
...
function hasil_lab_where($id_anamnesa,$status){
 ...
 $this->db->select('*');
 $this->db->from('lab_parasit');
 $this->db->join('disposisi','disposisi.id_anamnesa = lab_parasit.id_anamnesa ');
 $this->db->where('lab_parasit.id_anamnesa',$id_anamnesa);
 $this->db->where('disposisi.nama_distribusi','Lab Parasit');
