<?php 
class M_lab_jamur extends CI_Model{
...
$this->datatables->where('disposisi.nama_distribusi','Lab jamur');
...
function hasil_lab_where($id_anamnesa,$status){
 ...
 $this->db->select('*');
 $this->db->from('lab_jamur');
 $this->db->join('disposisi','disposisi.id_anamnesa = lab_jamur.id_anamnesa ');
 $this->db->where('lab_jamur.id_anamnesa',$id_anamnesa);
 $this->db->where('disposisi.nama_distribusi','Lab Jamur');
